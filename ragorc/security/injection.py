...
def render_untrusted_passages(texts: Sequence[str], *, start: int = 1) -> str:
    """Number and fence a list of retrieved passages for a prompt.

    The packer does this for the *answer* prompt, and only for that one. Every
    other query-time stage that reads retrieved text built its own
    ``"\n\n".join(...)`` and interpolated it raw — the groundedness grader, CRAG's
    relevance grader, RankGPT, both compressors, multi-hop reasoning, query
    rewriting and claim verification. So ``compress_extract`` rendered as::
    ...
    with the injected sentence at the same level as the system rules. The attacks
    are not theoretical and each promotes attacker text into the answer: an
    instruction that persuades the groundedness grader defeats the hallucination
    guard; one that persuades CRAG marks a document CORRECT; one that persuades
    RankGPT ranks it first. A compressor's output is then inserted into the answer
    prompt *as retrieved evidence*, having been laundered through a model.

    ``[n]`` stays outside the fence for the reason
    :meth:`~ragorc.context.pack.ContextPacker._render_passage` gives: it is an
    integer this function generated by counting, and it is what the model cites.
    """
    return "\n\n".join(
        f"[{index}]\n{wrap_untrusted(text.strip(), index=index)}"
